...
# ----- VOICEVOX音声（話者IDを指定。省略時はイッチ＝ナレーター） -----
def make_audio(text, filename, speaker=None):
    if speaker is None:
        speaker = SPEAKER_OP
...
def resolve_speakers():
    """/speakers から「話者名＋スタイル名」でstyle idを引き直す（IDズレ対策）"""
    global SPEAKER_OP, SPEAKER_NPC
    try:
        sp = requests.get(f"{VOICEVOX_URL}/speakers", timeout=30).json()
    except Exception as e:
        print(f"  /speakers取得失敗（既定IDのまま）: {e}")
        return

    def style_id(name, style_name):
        for s in sp:
    ...
    if op is not None: SPEAKER_OP = op
    else: print(f"  ⚠️ 『{SPEAKER_OP_NAME}』未検出。既定ID {SPEAKER_OP} を使用")
    if npc is not None: SPEAKER_NPC = npc
    else: print(f"  ⚠️ 『{SPEAKER_NPC_NAME}』未検出。既定ID {SPEAKER_NPC} を使用")
...
def build_video(threads):
    ...
    for ti, th in enumerate(threads):
        ...
        for r in th["res"]:
            ...
            speaker = SPEAKER_OP if is_op else SPEAKER_NPC
            print(f"  [スレ{ti+1} レス{res_no} {'イッチ' if is_op else '名無し'}] {body[:20]}...")
            a = make_audio(body, f"a_{idx}.mp3", speaker)
            dur = AudioFileClip(a).duration + 0.35
            p = f"{TMP_DIR}/clip_{idx:04d}.mp4"
            render_clip(make_res_clip(dur, th["title"], str(res_no), is_op, body), a, p)
            clip_paths.append(p); os.remove(a); idx += 1
